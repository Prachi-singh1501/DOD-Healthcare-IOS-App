...
function getservice()
{
    ...
    $.ajax({
        ...
        success: function (responce) {
            ...
            {
                services += '<div class="text-end d-grid mt-4"><div id="ribbon-container"><a href="#" id="ribbon">' + PackageType + '</a></div><button class="primary_btn"> Not Approved</button> </div></div></div></div></div></div>';
            }
            else {
                services += '<div class="text-end d-grid mt-4"><div id="ribbon-container"><a href="#" id="ribbon">' + PackageType + '</a></div><button class="btn btn-default shadow detail_btn CustomPackagePayment"> Buy Now</button> </div></div></div></div></div></div>';
            }
...
$(document).unbind().on("click", ".CustomPackagePayment", function () {
